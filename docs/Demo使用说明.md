...
1. 选择由完整 nuScenes 训练数据索引动态匹配出的“真实路口”“行人运动”“近距前车”或“转弯场景”，也可输入任意 `scene_id` 或随机抽取样本。
...
推荐先演示“近距前车”：观察前车持续运动、Baseline 风险和反思轨迹的动态安全间距；再切换“行人运动”，展示 annotation 轨迹、礼让评分与重规划。nuScenes mini 不含可直接使用的逐帧灯态，因此“真实路口”使用中性灯态先验，不把它作为真实红灯识别演示。

右侧“Skill 反思自进化”会同步展示当前场景画面、数据标识、触发条件和动作程序。播放轨迹时，生成流程会依次点亮失败捕获、反思归因、Skill 抽象、重规划验证和记忆固化；最后的分数增益来自本次 Critic 复评，历史支持数从反思记忆文件实时统计。
...
- 动态参与者：前车按场景速度持续行驶；行人位置和速度来自当前 sample 的 nuScenes `sample_annotation` 后继链，按真实时间戳插值并显示 annotation 来源。无行人标注则不显示，手工场景只有静态位置，不伪造横穿运动。反思后策略带有移动前车安全间距约束，自车停车时保持最后有效航向。
